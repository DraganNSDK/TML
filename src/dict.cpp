...
lexeme dict_t::get_sym(int_t t) const {
	DBG(assert(!(t&1) && !(t&2) && syms.size()>(size_t)((t>>2)-1));)
	static wchar_t str_nums[20], str_chr[] = L"'a'";
	if (t & 1) { str_chr[1] = (t>>=2)-1; return { str_chr, str_chr + 3 }; }
	if (t & 2) return wcscpy(str_nums, to_wstring((t>>=2)-1).c_str()),
...
int_t dict_t::get_sym(const lexeme& l) {
	...
	return syms.push_back(l), syms_dict[l] = (syms.size()-1+1)<<2;
}
