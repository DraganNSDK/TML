...
////	uints perm = perm_init(args * bits); // identity
...
////	// args-first and current pos/bits make this easy to do (smth to ponder on)
...
////	bdd_handles v;
////	// D: this bdd only 'sticks' for table for rules that aren't facts. 
...
////	ints t;
////	lexeme l;
////	size_t nvars = 0, nnums = 0, nchars = 0, nsyms = 0;
...
////	//vector<arg_type> newtypes;
////	//vector<arg_type>& types = realrel ? tbls[tab].bm.types : newtypes;
////	// D: we could match types/nums right below, but it's cleaner in setargs.
...
////				// D: don't calc bitness, only if needed (match/calc in setargs)
////				types.emplace_back(base_type::INT, 0);
////				++nnums;
////				////nums = max(nums, r.e[n].num);
////				//DBG(assert(!realrel || types.size() >= t.size()););
////				//size_t iarg = t.size()-1;
////				//if (realrel && types[iarg].type == base_type::INT) {
////				//	if (r.e[n].num >= 1 << types[iarg].bitness) {
////				//		types.emplace_back(base_type::INT, 0); // -1);
////				//	}
////				//} 
////				//else if (realrel && types[iarg].type != base_type::NONE)
////				//	parse_error(err_type_mismatch, r.e[n].e);
////				//else if (realrel) {
////				//	// base_type::NONE
////				//}
////				//else {
...
////				types.emplace_back(base_type::CHR, 0); // -1);
////				//chars = 255;
////				//types.push_back(dict.get_type(L"chr[8]"));
////				break;
////			case elem::VAR:
...
////				else
////					types.emplace_back(base_type::NONE, 0); // -1);
////				++nvars;
////				break;
////			case elem::STR:
////				l = r.e[n].e;
...
////				types.emplace_back(base_type::STR, 0); // -1);
////				//syms = dict.nsyms();
////				break;
////			case elem::SYM: 
...
////		DBG(assert(abs(y) == 1);)
////		term r(false, term::REL, NOP, tab, ints(len, 0), argtypes(len), 0);
...
////void alt::init_varmap(const term& h, const term_set& al) {
////	varmap m;
////	// D: copy all header types + add below the body vars' types.
////	ints ats = (ints)h;
////	vector<arg_type> atypes = h.types;
////	varslen = h.size();
...
////				m.emplace(t[n], varslen++);
////				ats.push_back(t[n]);
////				atypes.push_back(t.types[n]);
////			}
////	DBG(assert(varslen == ats.size() && varslen == atypes.size()););
////	vm = m;
////	inv = varmap_inv(vm);
////	// also initialize bitsmeta (types are only really needed for now).
////	// D: issue: what if terms type changes after this? is it possible? then 
////	// we'd need to also update this bm (I'm referring to table bm update).
////	bm.setargs(ats, atypes);
////	//return m;
////}
////
...
////	a.init_varmap(h, al); // we get varmaps, varslen, inv and bm set.
////
...
////	const bitsmeta& bm) const{
////	size_t lsum = bm.leftbits(arg, args); // same for all bits, TODO: move to bm
////	for (size_t k = 0; k != bits; ++k) 
////		r = r && from_bit(k, arg, args, sym, bm, lsum);
////}
////
...
////	table tb;
////	// D: TODO: move vector init to .ctor
////	tb.bm.types = vector<arg_type>(len);
////	tb.bm.vbits = tb.bm.vargs = vector<size_t>(len, 0);
////	for (size_t i = 0; i != len; ++i) tb.bm.vargs[i] = i; // native ordering
...
////	syms = dict.nsyms();
////	// D: add_bit is a problem for variable-bits, how do we do this?
////	// at this point we have all terms, tables set up, we don't have alts yet.
////	// we have terms, types, tables bm-s all set up - but w/o knowing nums/sizes
////	// - get nums, chars, syms - and all default types, universe sizes, bitness.
////	// - then loop through tables, types, bm-s and update the default types/bits
////	// - add 2 ex-bits for now, just till we test varbits first, then remove it.
////	// note: this is before any bdd, alts etc. i.e. it's setup/init time, so ok.
////	// table bdd init is also later (get_facts, get_rules), i.e. no bdds yet.
////	// nums, chars we already have (at the end to_terms), now we calc types/bits
////	// - from_raw_term: add nums (per table, per arg), merge, keep types, nums.
////	// - when done/here, sum up and create types, bitness.
////	// - go through args/bits and what add_bit does
////	// - adjust pos/algorithm to do bits properly, args or bits-first (add-bit).
////	// - add zeros (no need to permute, only for later add_bit) to tbl bdd-s.
////	// - also perm_init(?), clear some cache.
////	// - basically we need to set 'bits' per table, arg, know layout etc.
...
////
////template<typename T>
////inline size_t BitScanR(T n) { return for (size_t b=0; n>0; n>>=1) ++b, b; }
//////{
//////	for (size_t nbits = 0; num > 0; num >>= 1) ++nbits;
//////	return nbits;
//////	return for (size_t b = 0; n > 0; n >>= 1) ++b, b;
//////}
////
////bool bitsmeta::setargs(const ints& args, const argtypes& vtypes) {
...
////		types = vtypes;
////		// D: vbits are temp/cache really, we should do that later at table init
...
////			if (vtypes[i].type == base_type::NONE) continue; // not set, skip
////			if (types[i].type == base_type::NONE) // first init...
////				types[i] = vtypes[i];
////			else if (types[i].type == vtypes[i].type) {
////				// base types need to match
////				if (vtypes[i].bitness > types[i].bitness)
////					types[i].bitness = vtypes[i].bitness; // update if more
////			} 
////			else parse_error(err_type_mismatch, L""); // TODO: pass lexeme?
////			// calc bitness if needed...
////			switch (types[i].type) {
////				case base_type::INT:
////					int_t num = args[i] >> 2; // this'll go away soon
////					// D: what about just '0'? bits is 0 too? also 0 vs 'null'?
////					if (num == 0) num = 1; // just to force one bit at least.
////					if (num >= 1 << types[i].bitness) {
////						// BitScanR()
////						for (size_t nbits = 0; num > 0; num >>= 1) ++nbits;
////						types[i].bitness = nbits;
////					}
////					break;
////				case base_type::STR:
////					break;
...
////				default: ;
////			}
////			vbits[i] = types[i].bitness; // possibly at tbl init?
////		}
////	}
////	++nterms;
////	return true;
...
//	int_t c, size_t arg, size_t args, size_t bit, const bitsmeta& bm) const {
//	//size_t bits = bm.types[arg].bitness;
//	//size_t lsum = bm.leftbits(arg, args);
//	//for (size_t b = 0; b != bits; ++b)
//	//	r = r && bm.from_bit(b, arg, args, sym, lsum);
//
//	if (!--bit)
//		return (c & 1) ? htrue : bm.from_bit(0, arg, args, 0);
//	//::from_bit(bm.pos(0, arg, args), false);
//	return (c & (1 << bit)) ?
//		bdd_ite_var(bm.pos(bit, arg, args), leq_const(c, arg, args, bit, bm),
//			htrue) :
//		bdd_ite_var(bm.pos(bit, arg, args), hfalse,
//			leq_const(c, arg, args, bit, bm));
//
//	//if (!--bit)
//	//	return	(c & 1) ? htrue :
//	//		::from_bit(pos(0, arg, args), false);
...
////}
//
//spbdd_handle table::add_bit() {
//	spbdd_handle x = t;
//	size_t args = len;
//	return t;
//}
//
//spbdd_handle tables::add_bit(table& tbl) { //, spbdd_handle x, size_t args) {
...
//		size_t lsum = bm.leftbits(arg, args); // precalculate, same for all b-s 
//		//size_t bits = bm.vbits[arg]; // this is now variable 'bits' (per arg)
//		size_t bits = bm.types[arg].bitness; // D: this is now 'bits' (per arg)
//		for (size_t b = 0; b != bits; ++b)
//			v.push_back(bm.from_bit(b, arg, args, 0, lsum));
//		//v.push_back(::from_bit(pos(0, bits + 1, arg, args), false));
//	}
...
//	size_t lsum = bm.leftbits(arg, args); // precalculate, same for all bits
//	for (size_t b = 0; b != bits; ++b)
//		r = r && bm.from_bit(b, arg, args, i, lsum);
//	return smemo.emplace(x, r), r;
//}
//
//spbdd_handle tables::from_sym_eq(size_t p1, size_t p2, size_t args) const {
...
//	ints t, nums = ints(r.nargs, 0); // D: use -1 or numeric_limits<int>::min()?
//	lexeme l;
//	size_t nvars = 0; // , nnums = 0, nchars = 0, nsyms = 0;
//	term::textype extype = (term::textype)r.extype;
//	// D: header builtin is treated as rel, but differentiated later (decomp.)
...
//			//size_t bitness = !realrel ? bitsmeta::BitScanR(r.e[n].num) : 0;
//			//types.emplace_back(base_type::INT, bitness);
//			//++nnums;
//			////nums = max(nums, r.e[n].num);
//			//DBG(assert(!realrel || types.size() >= t.size()););
//			//size_t iarg = t.size()-1;
...
//		tbls[tab].bm.setargs(t, types);
//	//else
//	//	// D: temp only, just to calc bits - tbs if we need to keep term types
//	//	for (size_t i = 0; i != types.size(); ++i)
//	//		if (types[i].type == base_type::INT)
//	//			types[i].bitness = bitsmeta::BitScanR(t[i] >> 2);;
//
//	// D: idbltin name isn't handled above (only args, much like rel-s & tab-s).
//	if (extype == term::BLTIN) {
//		int_t idbltin = dict.get_bltin(r.e[0].e);
...
//	if (arg < 0) return elem(elem::VAR, get_var_lexeme(arg));
//	if (arg & 1) {
//		const wchar_t ch = arg >> 2; // un_mknum(arg);
//		if (iswprint(ch)) return elem(ch);
//		return	elem(elem::SYM, dict.get_lexeme(wstring(L"\"#") +
//			to_wstring((unsigned char)(ch)) + L"\""));
//	}
//	if (arg & 2) return elem((int_t)(arg >> 2)); // un_mknum(arg);
//	return elem(elem::SYM, dict.get_sym(arg));
//}
//
//raw_term tables::to_raw_term(const term& r) const {
...
//		for (const term& t : v)
//			for (int_t i : t)
//				if (i & 2) m = max(m, i >> 2); // un_mknum(arg);
//	for (vector<term>& t : x) {
//		freeze(t, m);
//		for (size_t n = 1; n != t.size(); ++n) p.insert({ t[n] });
//	}
...
//	size_t lsum = bm.leftbits(arg, args); // same for all bits, TODO: move to bm
//	for (size_t k = 0; k != bits; ++k)
//		r = r && bm.from_bit(k, arg, args, sym, lsum);
//}
//
//ntable tables::get_table(const sig& s) {
//	auto it = smap.find(s);
...
//	// D: TODO: move vector init to .ctor
//	tb.bm.types = vector<arg_type>(len);
//	tb.bm.vargs = vector<size_t>(len, 0);
//	//tb.bm.vbits = tb.bm.vargs = vector<size_t>(len, 0);
//	for (size_t i = 0; i != len; ++i) tb.bm.vargs[i] = i; // native ordering
//	return	tb.t = hfalse, tb.s = s, tb.len = len,
//		tbls.push_back(tb), smap.emplace(s, nt), nt;
...
//			DBG(assert(a.bltinargs.size() == 3););
//			// TODO: check that it's num const
//			int_t arg0 = int_t(a.bltinargs[0] >> 2); // un_mknum(arg);
//			int_t arg1 = int_t(a.bltinargs[1] >> 2); // un_mknum(arg);
//			if (arg0 > arg1) swap(arg0, arg1);
//
//			//int_t rnd = arg0 + (std::rand() % (arg1 - arg0 + 1));
//			std::random_device rd;
...
//				int_t arg = a.bltinargs[n++];
//				if (arg < 0) os << get_var_lexeme(arg) << endl;
//				else if (arg & 1) os << (wchar_t)(arg >> 2); // un_mknum(arg);
//				else if (arg & 2) os << (int_t)(arg >> 2); // un_mknum(arg);
//				else              os << dict.get_sym(arg);
//			} while (n < a.bltinargs.size());
//		}
//	}
...
//			bool isset = true;
//			if (type.type == base_type::NONE) { // first init...
//				type = newtype;
//				if (type.type == base_type::INT)
//					nums[i] = vnums[i];
//			}
//			else if (type.type == newtype.type) {
//				if (type.type == base_type::INT)
//					nums[i] = max(nums[i], vnums[i]);
//				if (newtype.bitness > type.bitness) // update if more only...
